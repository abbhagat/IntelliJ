 ...
 * Output : 5
 * Explanation :
 * There are five pairs possible whose sum is divisible by '4' i.e.,
 * (2, 2), (1, 7), (7, 5), (1, 3) and (5, 3)
 * <p>
...
public class CountPairsWithSumDivisibleByK {
    ...
    public static void main(String[] args) {
        int[] a = {2, 2, 1, 7, 5, 3};
        int k = 4;
        int[] freq = new int[k];  // freq[] to keep the count of the number of remainders
        for (int x : a) {
            freq[x % k]++;  // Count occurrences of all remainders
        }
        int sum = freq[0] * (freq[0] - 1) / 2;  // If both the pairs are divisible by K
