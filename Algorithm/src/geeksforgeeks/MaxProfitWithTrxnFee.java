...
public class MaxProfitWithTrxnFee {
    ...
    private static int max_profit(int a[], int b[], int n, int fee) {
        int i, j, profit, l, r, diff_day = 1, sum = 0;
        ...
        for (i = 1; i < n; i++) {
            ...
            sum = 0;
            for (j = n - 1; j >= i; j--) {
                profit = (a[r] - a[l]) - fee;  //here finding the max profit if we get less then or equal to zero it means we are not getting the profit
                if (profit > 0) {
                    sum = sum + profit;
                }
                l++;
