...
public class LargestContiguousProductArrayPrint {
    ...
    private static void maxSubarrayProduct(int a[]) {
        ...
        max_end is always 1 or some positive product ending with x
        min_end is always 1 or some negative product ending with x */
        for (int x : a) {
            ...
            } else if (x == 0) {   // If this element is 0, then the maximum product cannot end here,
            ...
            max_end * x next max_end will be 1 if prev min_end is 1, otherwise
            next max_end will be prev min_end * x */
            else {
                int temp = max_end;
                 max_end = max(min_end * x, 1);
                 min_end = temp * x;
            }
