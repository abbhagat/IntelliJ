...
public class FindMaxInARotatedSortedArray {
  ...
  private static int findMax(int[] a, int low, int mid, int high) {
    if (low <= high) {
      mid = (low + high) / 2;
      if (a[mid] == a[high]) {
        return findMax(a, low, mid, high - 1);
      }
      if (a[mid] > a[mid + 1]) {
        return a[mid];
      }
      return a[low] > a[mid] ? findMax(a, low, mid, mid - 1) : findMax(a, mid + 1, mid, high);
    }
    return a[mid];
  }
  ...
  public static void main(String[] args) {
    ...
    System.out.println("Max :" + findMax(a) + "\t" + findMax(a, 0, 0, a.length - 1));
