    ...
    private static int getMaxGold(int[][] gold, int m, int n) {
        ...
        for (int col = n - 1; col >= 0; col--) {
            for (int row = 0; row < m; row++) {
                ...
                goldTable[row][col] = gold[row][col] + Math.max(right, Math.max(right_up, right_down)); // Max gold collected from taking either of the above 3 paths
            }
        }
        int res = goldTable[0][0];
        for (int i = 1; i < m; i++) {
            res = Math.max(res, goldTable[i][0]);  // The max amount of gold collected will be the max value in first column of all rows
        }
        return res;
    }
